// 8. There is an old story that emperor wanted to thank the inventor of the
...
//    to keep track of wich square you are at, an int to keep the number of
//    grains on the current square, and an int to keep track of the grains on
//    all previous squares. We suggest that you write out the value of all
//    your variables for each iteration of the loop so that you can see what's
//    going on.

#include<iostream>

int requiredSquares(int numberOfGrains)
{
    int i = 0, sum = 0, grains = 1;
    while(sum < numberOfGrains)
    {
        i++;
        sum += grains;
        grains *= 2;
...
{
    std::cout << std::endl;
    std::cout << requiredSquares(1000) << std::endl;
    std::cout << requiredSquares(1000'000) << std::endl;
    std::cout << requiredSquares(1000'000'000) << std::endl;
}
